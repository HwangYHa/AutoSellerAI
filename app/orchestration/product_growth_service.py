...
A ProductGrowthWorkflow is the join point between product truth, detail assets,
Threads copy/media, click tracking and order attribution. Preparation is idempotent
where practical and external effects remain behind explicit queue/schedule actions.
"""
...
def _url_is_public(value: str) -> bool:
...
def tracking_is_public(link: TrackingLink | None) -> bool:
    return bool(link and _url_is_public(tracking_public_url(link)))
...
def prepare_threads_drafts(workflow_id: int, *, count: int = 3, force: bool = False) -> list[SocialContentDraft]:
    ...
    tracked_url = tracking_public_url(link) if tracking_is_public(link) else ""
    target_url = tracked_url or row.destination_url
...
def attach_image_generation(workflow_id: int, generation_id: int) -> dict[str, Any]:
    ...
    _merge_step(workflow_id, "social_visual", {"ok": True, "source": "stable_diffusion", "generation_id": int(generation_id), "staged": False})
...
def stage_attached_social_visual(workflow_id: int, image_index: int = 0) -> dict[str, Any]:
    ...
    is_public = media_base_is_public() and _url_is_public(public_url)
    _set_workflow(workflow_id, social_media_url=public_url, error="")
    _merge_step(
        workflow_id,
        "social_visual",
        {"ok": True, "source": "stable_diffusion", "generation_id": workflow.image_generation_id, "staged": True, "url": public_url, "public": is_public},
    )
    return {"filename": filename, "media_url": public_url, "public": is_public, "source": "stable_diffusion"}


def _select_product_asset(workflow_id: int, image_index: int, *, detail: bool) -> str:
    workflow = get_workflow(workflow_id)
    if not workflow:
        raise LookupError("workflow not found")
    with get_db() as db:
        ...
        if not product:
            raise LookupError("product not found")
        raw = product.detail_images if detail else product.images
        images = [str(x) for x in _loads(raw, []) if str(x).startswith(("http://", "https://"))]
...
def use_product_social_visual(workflow_id: int, image_index: int = 0) -> dict[str, Any]:
    url = _select_product_asset(workflow_id, image_index, detail=False)
    _set_workflow(workflow_id, social_media_url=url, error="")
    _merge_step(workflow_id, "social_visual", {"ok": True, "source": "product", "url": url, "public": _url_is_public(url)})
    return {"media_url": url, "public": _url_is_public(url), "source": "product"}


def use_detail_social_visual(workflow_id: int, image_index: int = 0) -> dict[str, Any]:
    url = _select_product_asset(workflow_id, image_index, detail=True)
    _set_workflow(workflow_id, social_media_url=url, error="")
    _merge_step(workflow_id, "social_visual", {"ok": True, "source": "detail", "url": url, "public": _url_is_public(url)})
    return {"media_url": url, "public": _url_is_public(url), "source": "detail"}
...
def register_detail_assets(workflow_id: int, image_urls: list[str], *, apply: bool = True) -> dict[str, Any]:
    ...
    if not workflow:
        raise LookupError("workflow not found")
    existing_workflow = [str(x) for x in _loads(workflow.detail_image_urls_json, []) if str(x).startswith(("http://", "https://"))]
    incoming = [str(x).strip() for x in image_urls if str(x).strip().startswith(("http://", "https://"))]
    urls = list(dict.fromkeys([*existing_workflow, *incoming]))
...
def _active_detail_job(workflow: ProductGrowthWorkflow, connection: Redis) -> str:
    step = _loads(workflow.steps_json, {}).get("detail_generation") or {}
    ...
    try:
        ...
        return job_id if status in _ACTIVE_RQ_STATES else ""
    except Exception:
        return ""
...
def queue_detail_generation(
    ...
    *,
    ...
    connection = _redis()
    connection.ping()
    active_job_id = _active_detail_job(workflow, connection)
    if active_job_id:
        return {"accepted": True, "job_id": active_job_id, "queue": "image", "reused": True}

    queue = Queue("image", connection=connection, default_timeout=int(os.getenv("IMAGE_AI_JOB_TIMEOUT_SECONDS", "900")))
    workers = Worker.all(queue=queue)
    if not workers:
        raise RuntimeError("image-worker가 실행 중이 아닙니다.")
    ...
    return {"accepted": True, "job_id": job.id, "queue": "image", "reused": False}
...
def schedule_workflow_post(
    ...
    *,
    ...
    with get_db() as db:
        ...
        existing = db.scalar(
            select(ScheduledSocialPost).where(
                ScheduledSocialPost.draft_id == draft.id,
                ScheduledSocialPost.campaign_key == workflow.campaign_key,
                ScheduledSocialPost.status.in_(_ACTIVE_SCHEDULE_STATES),
            )
        )
        if existing:
            db.expunge(existing)
    ...
    if include_tracking_url and link:
        if not tracking_is_public(link):
            raise ValueError("PUBLIC_BASE_URL must be a publicly reachable HTTP(S) URL before adding a Tracking Link to Threads")
        suffix = f"\n\n{tracking_url}"
    ...
    if source == "workflow":
        ...
        if media_url:
            if not _url_is_public(media_url):
                raise ValueError("Threads image media URL must be publicly reachable")
            media_type = "IMAGE"
    elif source == "product":
        visual = use_product_social_visual(workflow_id, 0)
        if not visual["public"]:
            raise ValueError("product image URL must be publicly reachable")
        media_url = visual["media_url"]
        media_type = "IMAGE"
    elif source == "detail":
        visual = use_detail_social_visual(workflow_id, 0)
        if not visual["public"]:
            raise ValueError("detail image URL must be publicly reachable")
        media_url = visual["media_url"]
        media_type = "IMAGE"
    elif source in {"none", "text"}:
        pass
    else:
        raise ValueError("media_source must be workflow, product, detail or none")
...
def workflow_to_dict(row: ProductGrowthWorkflow) -> dict[str, Any]:
    draft_ids = [int(x) for x in _loads(row.draft_ids_json, []) if str(x).isdigit()]
    with get_db() as db:
        product = db.get(Product, row.product_id)
        product_data = _product_dict(product) if product else None
        drafts = list(db.scalars(select(SocialContentDraft).where(SocialContentDraft.id.in_(draft_ids or [-1]))).all())
    ...
    generated_raw = _loads(row.detail_generated_json, [])
    generated_public = [
        {"role": x.get("role", ""), "public_url": x.get("public_url", "")}
        for x in generated_raw
    ...
    return {
        ...
        "detail": {
            ...
            "generated": generated_public,
        },
        "social_visual": {
            "image_generation_id": row.image_generation_id,
            "media_url": row.social_media_url,
            "public": _url_is_public(row.social_media_url),
            "threads_media_base_public": media_base_is_public(),
        },
        "tracking": {
            ...
            "public": tracking_is_public(tracking),
        },
        "drafts": [
...
__all__ = [
    "_merge_step", "_product_dict", "_set_workflow",
    ...
    "stage_attached_social_visual", "use_product_social_visual", "use_detail_social_visual",
    "register_detail_assets", "apply_detail_assets", "queue_detail_generation",
    "schedule_workflow_post", "tracking_public_url", "tracking_is_public",
]
