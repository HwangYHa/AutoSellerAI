...
def normalize_commerce_secret(value: str) -> str:
    """Normalize Docker escaped bcrypt secret used by Naver Commerce API."""
...
def commerce_secret_valid(value: str) -> bool:
    """Naver Commerce client secret is a 29-char bcrypt salt, not Search API secret."""
    secret = normalize_commerce_secret(value)
...
class SmartStoreUploader:
    def __init__(self):
        ...
        self._client_secret = normalize_commerce_secret(s.naver_client_secret)
    ...
    def _ensure_token(self) -> None:
        ...
        if not self._client_id:
            raise ValueError("NAVER_CLIENT_ID가 비어 있습니다. 네이버 Commerce API 애플리케이션의 Client ID를 입력하세요.")
        if not commerce_secret_valid(self._client_secret):
            raise ValueError(
                "NAVER_CLIENT_SECRET 형식이 잘못되었습니다. "
                "스마트스토어 Commerce API Client Secret은 29자리 bcrypt salt($2a$/$2b$/$2y$...)여야 합니다. "
                "네이버 검색 API의 Client Secret은 NAVER_SEARCH_CLIENT_SECRET에 별도로 입력하세요."
            )
        import bcrypt
        ts = str(int(time.time() * 1000))
        pw = f"{self._client_id}_{ts}"
        hashed = bcrypt.hashpw(pw.encode(), self._client_secret.encode())
        ...
        if r.status_code not in (200, 201):
            raise ValueError(f"네이버 Commerce API 인증 실패 HTTP {r.status_code}: {r.text[:300]}")
        data = r.json()
    ...
    def create_product(self, product: dict) -> dict:
        ...
        if not rep_images:
            raise ValueError("스마트스토어: 대표 이미지 CDN 업로드 실패\nIMAGE API 권한이 필요합니다. 네이버 판매자센터 > API 관리 > 권한 신청")
        detail_html = product.get("detail_html", "")
        if detail_images:
            img_block = "\n".join(f'<img src="{u}" style="max-width:860px;display:block;margin:0 auto 4px;">' for u in detail_images)
        ...
        payload: dict[str, Any] = {
            "originProduct": {
                "statusType": "SALE", "saleType": "NEW", "leafCategoryId": category_id,
                "name": product["name"][:100],
                "images": {"representativeImage": {"url": rep_images[0]}, "optionalImages": [{"url": u} for u in rep_images[1:]]},
                "detailContent": detail_html,
                "salePrice": (int(product["sell_price"]) // 10) * 10 or 10,
                "stockQuantity": product.get("stock", 999),
                "deliveryInfo": {
                    "deliveryType": "DELIVERY", "deliveryAttributeType": "NORMAL", "deliveryCompany": self._delivery_code,
                    "deliveryFee": {"deliveryFeeType": fee_type, **({"baseFee": shipping_fee or 2500, "deliveryFeePayType": "PREPAID"} if fee_type != "FREE" else {})},
                    "claimDeliveryInfo": {
                ...
                "detailAttribute": {
                    "naverShoppingSearchInfo": {"modelName": product.get("sku", ""), "manufacturerName": brand, "brandName": brand, "catalogMatchingYn": "N"},
                    "afterServiceInfo": {"afterServiceTelephoneNumber": self._after_service_phone, "afterServiceGuideContent": "상품 관련 문의는 고객센터로 연락 주세요."},
                    "originAreaInfo": {"originAreaCode": area_code, "content": origin_raw, "importer": "수입사 미상", "plural": False},
                    "taxType": "TAX", "minorPurchasable": True,
                    "productInfoProvidedNotice": {
                        "productInfoProvidedNoticeType": "ETC",
                        "etc": {
                            "returnCostReason": "상품 이상 시 환불", "noRefundReason": "변심 반품 불가 (상품 특성상)",
                            "qualityAssuranceStandard": "제조사 기준", "compensationProcedure": "고객센터 접수",
                            "troubleShootingContents": "고객센터 문의", "itemName": product["name"][:50],
                            "modelName": product.get("sku", ""), "certificateDetails": "해당없음", "manufacturer": brand,
    ...
    def _build_options(options_raw: list[dict], stock: int) -> dict | None:
        ...
        if len(values_list) == 1:
            for v in values_list[0]:
                ...
                if clean_v:
                    combinations.append({"stockQuantity": max(1, stock // max(len(values_list[0]), 1)), "price": 0, "usable": True, "optionName1": clean_v})
            ...
            for v1 in values_list[0]:
                for v2 in values_list[1]:
                    ...
                    combinations.append({"stockQuantity": max(1, stock // n), "price": 0, "usable": True, "optionName1": cv1, "optionName2": cv2})
        if not combinations:
            return None
        return {"optionCombinationSortType": "CREATE", "optionCombinationGroupNames": groups, "optionCombinations": combinations, "useStockManagement": True}

    def get_orders(self, from_date: str = "", to_date: str = "", pay_status: str = "PAYMENT_DONE", limit: int = 100) -> list[dict]:
        if not from_date or not to_date:
            from datetime import datetime, timedelta
            now = datetime.now(); from_date = (now - timedelta(days=2)).strftime("%Y%m%d"); to_date = now.strftime("%Y%m%d")
        self._ensure_token()
        params = {"dispatchFrom": from_date, "dispatchTo": to_date, "paymentDateType": "PAY_DATE", "payStatus": pay_status, "page": 1, "size": limit}
        try:
            r = httpx.get(f"{API}/v1/pay-order/vendor/orders", params=params, headers=self._headers(), timeout=20)
            if r.status_code == 200:
                data = r.json(); orders_raw = data.get("data", {}).get("contents", []) or data.get("contents", []); results = []
                for order in (orders_raw if isinstance(orders_raw, list) else []):
                    order_id = str(order.get("orderId", ""))
                    for po in order.get("productOrders", []):
                        results.append({
                            "orderId": order_id, "productOrderId": str(po.get("productOrderId", "")),
                            "originProductNo": str(po.get("product", {}).get("originProductNo", "")), "productName": po.get("product", {}).get("name", ""),
                            "quantity": int(po.get("quantity", 1)), "unitPrice": float(po.get("unitPrice", 0)),
                            "buyerName": order.get("orderer", {}).get("name", ""), "receiverName": order.get("deliveryAddress", {}).get("name", ""),
                            "receiverAddr": (order.get("deliveryAddress", {}).get("roadAddress", "") + " " + order.get("deliveryAddress", {}).get("detailAddress", "")).strip(),
                            "receiverPhone": order.get("deliveryAddress", {}).get("tel1", ""), "shippingMessage": order.get("deliveryAddress", {}).get("message", ""),
    ...
    def dispatch_product_order(self, product_order_id: str, delivery_company: str, tracking_number: str) -> dict:
        self._ensure_token()
        body = {"dispatchProductOrders": [{"productOrderId": product_order_id, "deliveryMethod": "PARCEL", "deliveryCompanyCode": delivery_company, "trackingNumber": tracking_number, "dispatchDate": __import__("datetime").datetime.now().strftime("%Y-%m-%dT%H:%M:%S")}]}
        try:
            r = httpx.post(f"{API}/v1/pay-order/vendor/product-orders/dispatch", headers=self._headers(), json=body, timeout=20)
    ...
    def list_origin_products(self, max_pages: int = 20, page_size: int = 50) -> list[dict]:
        ...
        for page in range(1, max_pages + 1):
            r = httpx.get(f"{API}/v2/products", headers=self._headers(), params={"page": page, "size": page_size}, timeout=20)
            if r.status_code != 200: raise ValueError(f"스마트스토어 상품목록 조회 실패 HTTP {r.status_code}: {r.text[:300]}")
            data = r.json(); page_items = data.get("contents") or data.get("content") or []
            if not page_items: break
            for item in page_items:
                channel_products = item.get("channelProducts") or []; origin = channel_products[0] if channel_products else item
                results.append({"originProductNo": str(item.get("originProductNo") or origin.get("originProductNo") or ""), "name": origin.get("name") or item.get("name", ""), "salePrice": origin.get("salePrice") or item.get("salePrice", 0)})
