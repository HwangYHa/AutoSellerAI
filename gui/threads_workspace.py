...
def render() -> None:
    ...
    st.markdown(
        ...
        <div class="threads-hero"><h2>🧵 Social Commerce → Threads</h2>
        <p>AI 콘텐츠 → 게시·예약 → 댓글 영업 → Tracking → SmartStore/Coupang 구매 귀속을 한 곳에서 관리합니다.</p></div>
        """,
        unsafe_allow_html=True,
    )

    tabs = st.tabs([
        "📊 Dashboard", "✨ 콘텐츠", "📝 게시물", "💬 댓글", "🔥 HOT Leads",
        "🤖 AI Sales Inbox", "⚙️ 자동화", "🔐 API 설정",
    ])
    ...
    with tab_dash:
        ...
        for col, (icon, value, label) in zip(cols, [
            ...
            ("🔥", metrics["hot"], "HOT"), ("✅", metrics["sent"], "발행답글"),
            ("✨", metrics["drafts"], "콘텐츠"), ("🗓️", metrics["scheduled"], "예약"),
            ("🔗", metrics["clicks"], "클릭"), ("🛒", metrics["orders"], "귀속주문"),
        ]):
            col.metric(f"{icon} {label}", value)
        attr = attribution_summary()
        a, b, c = st.columns(3)
        a.metric("귀속 매출", f"{attr.get('attributed_revenue', 0):,.0f}원")
        b.metric("평균 Confidence", f"{attr.get('avg_confidence', 0)*100:.1f}%")
        token = credential_status()
        c.metric("Threads 토큰", f"{token.get('days_remaining', '-')}일" if token.get("connected") else "미연결")

    # ── Content AI + schedule + tracking ─────────────────────────────
    with tab_content:
        ...
        if not products:
            ...
            with c1:
                ...
                angle = st.selectbox("콘텐츠 각도", ["problem_solution", "experience", "question", "comparison", "listicle"])
            ...
            with c2:
                ...
                for d in drafts:
                    with st.container(border=True):
                        st.caption(f"#{d.id} · {d.angle} · score {d.score:.0f} · {d.ai_source}")
                        st.write(d.body)

        st.divider()
        st.subheader("Tracking URL + 게시 예약")
        left, right = st.columns(2, gap="large")
        with left:
            if products:
                track_pid = st.selectbox("Tracking 상품", [p.id for p in products], format_func=lambda x: _product_label(product_map[x]), key="track_pid")
                platform = st.selectbox("목적 플랫폼", ["smartstore", "coupang"])
                destination = st.text_input("실제 상품 URL", placeholder="https://...")
                campaign = st.text_input("Campaign Key", placeholder="threads-202608-carclean")
                if st.button("🔗 Tracking URL 생성", use_container_width=True):
                    try:
        ...
        with right:
            ...
            if drafts and products:
                ...
                link_id = st.selectbox("Tracking Link", [None] + [x.id for x in links], format_func=lambda x: "없음" if x is None else f"#{x}")
                media_type = st.selectbox("미디어", ["TEXT", "IMAGE", "VIDEO", "CAROUSEL"], key="schedule_media")
                media_url = st.text_input("이미지/영상 공개 URL", disabled=media_type not in {"IMAGE", "VIDEO"})
                alt_text = st.text_input("Alt text", disabled=media_type == "TEXT")
                carousel_json = st.text_area("Carousel JSON", value='[{"media_type":"IMAGE","image_url":"https://..."},{"media_type":"IMAGE","image_url":"https://..."}]', disabled=media_type != "CAROUSEL")
    ...
    with tab_posts:
        ...
        with left:
            media_type = st.selectbox("게시 형식", ["TEXT", "IMAGE", "VIDEO", "CAROUSEL"], key="publish_media")
            text = st.text_area("본문", max_chars=500, height=160)
            media_url = st.text_input("이미지/영상 공개 URL", disabled=media_type not in {"IMAGE", "VIDEO"}, key="publish_url")
            alt_text = st.text_input("Alt text", disabled=media_type == "TEXT", key="publish_alt")
            carousel_json = st.text_area("Carousel JSON", value='[]', disabled=media_type != "CAROUSEL", key="publish_carousel")
            ...
            if st.button("🧵 Threads에 지금 발행", type="primary", use_container_width=True):
                try:
        ...
        with right:
            ...
            st.markdown("#### 게시 이력")
            st.dataframe([{"ID":p.id,"내용":p.content[:80],"Threads ID":p.threads_post_id,"발행":p.published_at} for p in posts], use_container_width=True, hide_index=True)
            st.markdown("#### 예약 현황")
            st.dataframe([{"ID":s.id,"형식":s.media_type,"상태":s.status,"예약(KST)":s.scheduled_at.replace(tzinfo=ZoneInfo('UTC')).astimezone(KST).strftime('%m-%d %H:%M'),"오류":s.error[:80]} for s in schedules], use_container_width=True, hide_index=True)

    # ── Comments ──────────────────────────────────────────────────────
    with tab_comments:
        st.subheader("댓글 관리")
        a, b, c = st.columns(3)
        intent = a.selectbox("Intent", ["전체", "PURCHASE_INTENT", "SHIPPING", "STOCK", "PRICE", "COMPATIBILITY", "PRODUCT_INFO", "COMPLAINT", "RETURN", "UNKNOWN"])
        ...
        with get_db() as db:
            ...
            if intent != "전체": stmt = stmt.where(ThreadsComment.intent == intent)
            if human: stmt = stmt.where(ThreadsComment.requires_human.is_(True))
            rows = list(db.scalars(stmt.order_by(desc(ThreadsComment.received_at)).limit(300)).all())
        st.dataframe([{"사용자":r.author_username or r.author_id,"댓글":r.comment_text,"Intent":r.intent,"구매의도":f"{r.purchase_intent_score*100:.0f}%","사람확인":"⚠️" if r.requires_human else "","수신":r.received_at} for r in rows], use_container_width=True, hide_index=True)
    ...
    with tab_leads:
        ...
        for lead in leads:
            ...
            with st.container(border=True):
                st.markdown(f"**{icon} {lead.purchase_intent_score*100:.0f}% · @{lead.author_username or lead.author_id or 'unknown'} · {lead.intent}**")
    ...
    with tab_inbox:
        ...
        if not inbox: st.info("검수할 답글이 없습니다.")
        for reply, comment in inbox:
            with st.expander(f"{comment.purchase_intent_score*100:.0f}% · @{comment.author_username or comment.author_id or 'unknown'} · {comment.intent}", expanded=comment.requires_human or comment.purchase_intent_score >= .85):
                st.write(comment.comment_text)
                if comment.requires_human: st.warning("민감 문의: 사람 확인이 필요합니다.")
                ...
                if s1.button("💾 저장", key=f"save_{reply.id}", use_container_width=True):
                    with get_db() as db:
                        row = db.get(ThreadsReply, reply.id); row.reply_text = edited.strip(); row.source = "human"; db.commit()
    ...
    with tab_rules:
        ...
        with left:
            with st.form("rule_form", clear_on_submit=True):
                ...
                save = st.form_submit_button("Rule 저장", type="primary", use_container_width=True)
            if save and keyword.strip() and template.strip():
                with get_db() as db:
                    ...
                    if exists: st.error("같은 키워드가 이미 있습니다.")
                    else:
                        db.add(ThreadsAutomationRule(keyword=keyword.strip(), reply_template=template.strip(), priority=int(priority), enabled=True)); db.commit(); st.success("저장했습니다."); st.rerun()
        with right:
            with get_db() as db: rules = list(db.scalars(select(ThreadsAutomationRule).order_by(ThreadsAutomationRule.priority)).all())
            for r in rules:
                with st.container(border=True):
                    c1, c2, c3 = st.columns([4,1,1]); c1.markdown(f"**{'🟢' if r.enabled else '⚪'} `{r.keyword}`** · {r.priority}"); c1.caption(r.reply_template)
                    if c2.button("ON/OFF", key=f"rt_{r.id}"):
                        with get_db() as db: row=db.get(ThreadsAutomationRule,r.id); row.enabled=not row.enabled; db.commit()
                        st.rerun()
                    if c3.button("삭제", key=f"rd_{r.id}"):
                        with get_db() as db: row=db.get(ThreadsAutomationRule,r.id); db.delete(row); db.commit()
                        st.rerun()

    # ── API / OAuth settings ──────────────────────────────────────────
    with tab_api:
        st.subheader("Meta Threads API 설정")
        status = credential_status()
        if status.get("connected"):
            a,b,c,d = st.columns(4)
            a.metric("계정", f"@{status.get('username') or status.get('threads_user_id')}")
            b.metric("토큰 잔여", f"{status.get('days_remaining')}일")
            c.metric("상태", status.get("status"))
            d.metric("만료", (status.get("expires_at") or "")[:10])
            ...
            if st.button("♻️ 60일 토큰 갱신", type="primary"):
                try: st.success(f"갱신 완료 · 잔여 {refresh_stored_credential().get('days_remaining')}일"); st.rerun()
                except Exception as exc: st.error(str(exc))
        else:
            st.warning("연결된 Threads OAuth 계정이 없습니다.")
            ...
            try:
                ...
                st.link_button("🔗 Meta Threads 계정 연결", result["authorization_url"], type="primary")
            except Exception as exc:
                st.info("Social API가 실행된 뒤 OAuth 연결 버튼이 활성화됩니다.")
                st.caption(str(exc))
        ...
        st.dataframe([{"설정":k,"상태":"✅ 설정됨" if v else "❌ 미설정"} for k,v in checks.items()], use_container_width=True, hide_index=True)
        st.caption("Access Token은 OAuth 완료 후 암호화되어 DB에 저장되며 화면에는 원문을 노출하지 않습니다.")
