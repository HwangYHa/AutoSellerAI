...
with left:
    st.markdown('<div class="area"><h3>⚡ 통합 운영</h3><div class="muted">기존 AutoSellerAI의 대시보드, 파이프라인, 상품, 검색 최적화, 주문, 정산, 재고, 알림, 스케줄러, 설정을 그대로 사용합니다.</div></div>', unsafe_allow_html=True)
    st.page_link("pages/00_AutoSeller_Main.py", label="통합 운영 화면 열기", icon="➡️", use_container_width=True)

with right:
    st.markdown('<div class="area"><h3>🛒 소셜커머스 → 스레드</h3><div class="muted">현황판 · 콘텐츠 · 게시물 · 댓글 · 구매 가능 고객 · AI 답글함 · 자동화 · API 설정과 순이익 기반 학습을 관리합니다.</div></div>', unsafe_allow_html=True)
...
for col, title, desc in zip(
    ...
    ["Meta OAuth / 60일 토큰", "상품 DB 기반 생성", "텍스트·이미지·영상", "댓글 분류·답글", "스마트스토어·쿠팡", "콘텐츠 점수 → 다음 전략"],
):
    with col:
        st.markdown(f"**{title}**")
        st.caption(desc)
