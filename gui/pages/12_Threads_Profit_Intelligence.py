"""Threads Profit Intelligence — 순이익 기반 Content Score / 전략 피드백."""
...
from app.social.threads.profit_feedback import learning_context, profit_dashboard, rebuild_profit_feedback
from app.social.threads.profit_models import ContentProfitSnapshot, ContentStrategyProfile

st.set_page_config(page_title="Threads Profit Intelligence | AutoSeller AI", page_icon="💹", layout="wide")
init_db()

st.markdown("# 💹 Threads Profit Intelligence")
st.caption("조회수가 아니라 실제 귀속 순이익을 기준으로 게시물·캠페인을 평가하고 다음 AI 콘텐츠 전략에 되먹임합니다.")

c1, c2 = st.columns([1, 4])
if c1.button("🔄 손익·Score 재계산", type="primary", use_container_width=True):
    with st.spinner("Attribution/정산 데이터를 기반으로 다시 계산 중..."):
        result = rebuild_profit_feedback()
    st.success(f"스냅샷 {result['snapshots']}건 / 전략 프로필 {result['profiles']}건 갱신")
    st.rerun()
c2.info("Scheduler도 기본 15분마다 자동 갱신합니다. 실제 Order 정산 데이터가 있으면 그 순이익을 우선 사용하고, 없을 때만 보수 추정합니다.")
...
k3.metric("Tracking 클릭", f"{summary['total_clicks']:,}")
...
with post_tab:
    rows = summary["items"]
    if not rows:
        st.info("아직 계산 가능한 게시물 손익 데이터가 없습니다. Tracking 클릭과 귀속 주문이 쌓이면 자동 생성됩니다.")
    else:
        st.dataframe([
            {
                "Threads ID": r["threads_post_id"],
                "캠페인": r["campaign_key"],
                "각도": r["angle"],
                "클릭": r["clicks"],
                "주문": r["orders"],
                ...
                "Content Score": f"{r['content_score']:.1f}",
                "정산품질": r["finance_quality"],
            }
            for r in rows
        ...
        st.success(
            f"현재 순이익 1위: Threads {best['threads_post_id'] or best['post_id']} · "
            f"순이익 {best['net_profit']:,.0f}원 · Content Score {best['content_score']:.1f}"
        )
        ...
        st.dataframe([
            {
                ...
                "PPC": f"{r.profit_per_click:,.0f}원",
                "Score": f"{r.content_score:.1f}",
                "정산품질": r.finance_quality,
            }
            for r in campaigns
        ], use_container_width=True, hide_index=True)
    ...
    if not profiles:
        ...
        for p in profiles:
            ...
            with st.expander(f"{product.name if product else p.profile_key} · 누적 순이익 {p.total_net_profit:,.0f}원", expanded=False):
                ...
                c.metric("평균 Score", f"{p.avg_content_score:.1f}")
                d.metric("Auto Apply", "ON" if p.auto_apply else "OFF")
                st.write("**우선 각도:**", ", ".join(preferred) if preferred else "아직 없음")
                st.write("**회피 각도:**", ", ".join(avoid) if avoid else "아직 없음")
                if patterns:
                    st.dataframe(patterns, use_container_width=True, hide_index=True)
                st.caption("다음 AI 콘텐츠 생성 시 이 프로필이 자동으로 프롬프트에 주입됩니다. 주문 표본이 적으면 과적합 방지를 위해 영향력이 제한됩니다.")
